...
from src.mydata import LoadDataset
...
config_path = "F:/project_code_2/UFDN/config/face_hd.yaml"
...
if trainer_conf['save_checkpoint']:
    model_path = conf['exp_setting']['checkpoint_dir'] + exp_name+'/' + trainer_conf['train_name'] + '/'
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    model_path = model_path+'{}'

if trainer_conf['save_log'] or trainer_conf['save_fig']:
    if os.path.exists(conf['exp_setting']['log_dir']+exp_name + '/' + trainer_conf['train_name']):
        shutil.rmtree(conf['exp_setting']['log_dir']+exp_name + '/' +trainer_conf['train_name'])
    writer = SummaryWriter(conf['exp_setting']['log_dir']+exp_name + '/' + trainer_conf['train_name'])
...
# Load dataset
domain_a = conf['exp_setting']['domain_a']  # 'three_d'
doamin_b = conf['exp_setting']['doamin_b']  # 'sur'
doamin_c = conf['exp_setting']['doamin_c']  # 'h_quality'
...
while global_step < trainer_conf['total_step']:    
    for a_img,b_img,c_img in zip(a_loader,b_loader,c_loader):
        ...
        d_pix_loss = pix_code_loss + 2 * pix_fake_pred -  2 * pix_real_pred + gp
        d_pix_loss.backward()
        ...
        if trainer_conf['save_log'] and (global_step % trainer_conf['verbose_step'] ==0):
            writer.add_scalar('MSE', mse.data.item(), global_step)
            writer.add_scalar('KL',  kl.data.item(), global_step)
            writer.add_scalar('gp', gp.data.item(), global_step)
            writer.add_scalars('Pixel_Distance',{'real':pix_real_pred.data.item(),
                                               'fake':pix_fake_pred.data.item()}, global_step)
            writer.add_scalars('Code_loss',{'feature':df_loss.data.item(),
                                            'pixel':pix_code_loss.data.item(),
                                           'adv_feature':feature_loss.data.item(),
                                           'adv_pixel':pix_clf_loss.data.item()}, global_step)
        ...
        if global_step% trainer_conf['plot_step'] ==0:
            fig_path = "vis/" + trainer_conf['train_name']
            # print(fig_path)
            ...
            save_fig1 = fig1 * 255
            cv2.imwrite("vis/%s/reconstruct_%d.jpg" % (trainer_conf['train_name'], global_step), save_fig1)
            ...
            save_fig2 = fig2 * 255
            cv2.imwrite("vis/%s/generate_%d.jpg" % (trainer_conf['train_name'], global_step), save_fig2)
